...
│   └── contexts/          # React上下文
├── doc/                   # 项目文档
│   └── xiaohongshu-clone-开发手册.md # 开发手册
...
- **`src/app/`**: 使用 Next.js App Router 进行路由管理
    - **`(home)/`**: 使用路由组隔离主页相关路由，避免布局嵌套问题
    - **`api/`**: 后端 API 路由实现

### 2. 组件组织

- **`src/components/`**: 按功能和用途分类组织组件
    - **`ui/`**: 基础 UI 组件，主要来自 shadcn/ui
    - **`layout/`**: 页面布局相关组件，如导航栏、侧边栏等
    - **`content/`**: 内容展示相关组件
    - **`navigation/`**: 导航相关组件
...
- **`src/lib/`**: 工具函数和通用逻辑
- **`src/services/`**: 服务层，处理数据获取和业务逻辑
- **`src/contexts/`**: React 上下文，用于状态管理

### 4. 配置文件

- **根目录下的配置文件**: 各种工具和框架的配置，如 ESLint、TypeScript、Next.js 等
